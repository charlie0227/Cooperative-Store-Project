...
<style>
		...
		{?>
			...
			<?if($result_img){?>
			<a href="http://people.cs.nctu.edu.tw/~cwchen05030530/<?echo $result_img->image_url?>"><img id="company_img" src="<?echo $result_img->image_url?>"/></a>
			<?}?>
	...
	if($_SESSION['id']){
		if($situation==0){?>
			...
			<input type="button" value="加入此團體" onclick="apply(<?echo $_SESSION['id']?>,<?echo$_GET['company_id']?>)"></a>
		<?}if($situation==1){?>
			<p>等待審核中...</p>
		<?}if($situation==2){?>
